...
pub fn discover_dsh() -> Vec<String> {
    ...
    if let Some(home) = std::env::var_os("DSH_HOME") { homes.push(PathBuf::from(home)); }
    if let Some(home) = std::env::var_os("HOME").or_else(|| std::env::var_os("USERPROFILE")) { homes.push(PathBuf::from(home).join(".dsh")); }
    ...
    let mut found: Vec<String> = candidates.into_iter().filter(|p| p.is_file()).filter_map(|p|p.canonicalize().ok()).map(|p|p.to_string_lossy().into_owned()).collect();
    found.sort(); found.dedup(); found
}

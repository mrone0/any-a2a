...
$('run-button').onclick = async () => {
  ...
  try {
    const response = await invoke('run_card', {id, message: $('message').value, configuration: requestConfiguration()});
    $('response-json').textContent = JSON.stringify(response, null, 2);
    $('result').textContent = response.error || response.text || '任务已返回，但没有文本回复。';
    $('result').classList.toggle('response-error', Boolean(response.error));
    $('response-meta').textContent = `${response.error ? '失败' : '已返回'} · ${((performance.now() - started) / 1000).toFixed(1)} 秒`;
  } catch (error) {
...
function renderClientEntrances() {
  ...
  for (const [id, name] of [['claude', 'Claude Code'], ['codex', 'Codex'], ['pi', 'pi'], ['dsh', 'DeepSeek Harness'], ['opencode', 'OpenCode'], ['hermes', 'Hermes']]) {
    ...
    detail.textContent = id === 'dsh' ? '支持 CLI 或 pnpm 源码启动；使用实际 profile 的 patch 路径配置，不要求全局 dsh 命令。' : '后续版本支持';
    const action = document.createElement('button'); action.className = 'secondary';
    action.textContent = id === 'dsh' ? '配置 DSH' : '暂未开放'; action.disabled = id !== 'dsh';

...
fn read() -> Result<String, String> {
    ...
    if !meta.is_file() || meta.len() > 8 * 1024 * 1024 {
        return Err("Expected a regular configuration file under 8 MiB".into());
    }
...
fn validate(text: &str) -> Result<(), String> {
        ...
        .filter(|(_, l)| !l.trim().is_empty())
    {
        let check = || -> Result<(), String> {
            ...
            if v["id"].as_str().is_none_or(|s| s.trim().is_empty()) {
            ...
            match v["source"].as_str() {
                Some("manual") => {
                    any_a2a::inspect_card(&v["agentCard"])?;
                }
                Some("url") => {
                    let url = reqwest::Url::parse(v["cardUrl"].as_str().ok_or("Missing cardUrl")?)
                        .map_err(|_| "Invalid URL")?;
                    if !matches!(url.scheme(), "http" | "https")
                        || !url.username().is_empty()
                        || url.password().is_some()
                    {
                        return Err("Invalid card URL".into());
                    }
                }
                _ => return Err("source must be url or manual".into()),
            }
            let auth: any_a2a::auth::Auth =
                serde_json::from_value(v.get("auth").cloned().unwrap_or(json!({})))
...
pub fn save_config(content: String, original: String) -> Result<Value, String> {
    ...
    validate(&content)?;
    if read()? != original {
        return Err("文件已发生变化，请重新加载后编辑".into());
    }
    let path = path()?;
    ...
    if read()? != original {
        return Err("文件在保存期间被修改，请重新加载".into());
    }

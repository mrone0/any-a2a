...
pub fn scan_clients() -> Vec<Client> {
    ...
    if let Some(home) = std::env::var_os("HOME").or_else(|| std::env::var_os("USERPROFILE")) {
        ...
        for suffix in [".local/bin", ".cargo/bin", ".bun/bin", ".opencode/bin", ".hermes/venv/bin"] {
    ...
    [("claude", "Claude Code"), ("codex", "Codex"), ("pi", "pi"), ("dsh", "DeepSeek Harness"), ("opencode", "OpenCode"), ("hermes", "Hermes")]
        .into_iter().map(|(id, name)| {
            let found = dirs.iter().flat_map(|dir| suffixes.iter().map(move |suffix| dir.join(format!("{id}{suffix}"))))
                .find(|path| executable(path));
            Client { id, name, detected: found.is_some(), executable: found.map(|p| p.to_string_lossy().into_owned()), integration: "not_verified" }
        }).collect()
}
...
mod tests {
    ...
    fn six_clients_without_activation_claims() {
        ...
        for client in clients { assert_eq!(client.detected, client.executable.is_some()); assert_eq!(client.integration, "not_verified"); }
    }
}
